...
# Skill: LeoMaster (Orquestração Multi-Agente)
...
- `.agent/skills/leo-master/.antigravity/equipe/registro_atividades.json` → Registro mestre de atividades, estados e pré-requisitos.
- `.agent/skills/leo-master/.antigravity/equipe/caixa_entrada/` → Comunicações individuais entre agentes (.msg).
- `.agent/skills/leo-master/.antigravity/equipe/aviso_geral.msg` → Comunicados globais para todo o esquadrão.
- `.agent/skills/leo-master/.antigravity/equipe/travas/` → Semáforos para impedir edição simultânea de arquivos.

# IMPORTANTE
1. Nunca crie uma nova pasta `.antigravity` na raiz do projeto.
Utilize a pasta `.antigravity` que fica dentro do diretório desta skill (`.agent/skills/leo-master/.antigravity/`).
...
1. **Condutor (LeoMaster)**: O líder do time. Decompõe o problema, distribui responsabilidades e valida planos de ação.
...
Antes de efetuar alterações relevantes, cada agente deve submeter um **Plano de Ação** à caixa de entrada do LeoMaster.

- O agente permanece em modo `SOMENTE_LEITURA` ou `PLANEJAMENTO` até que o LeoMaster responda com um comunicado de `APROVADO`.

### 2. Comunicação e Difusão (Broadcast)

- **Comunicação Direta**: Coordenação 1-a-1 entre executores.
- **Comunicado Geral**: LeoMaster pode escrever em `aviso_geral.msg` para transmitir novas orientações a todo o esquadrão simultaneamente.
...
- NUNCA editar um arquivo se existir um .lock ativo em `.agent/skills/leo-master/.antigravity/equipe/travas/`.
- Ao finalizar uma atividade, o agente deve liberar suas "travas" e notificar o LeoMaster.
