"""
Gerenciador de Esquadrão LeoMaestro
========================================
...
PASTA_ESQUADRAO = ".agent/skills/leo-maestro/.antigravity/equipe"
...
class GerenciadorEsquadrao:
    ...
    def preparar_infraestrutura(self) -> None:
        ...
        print("[OK] Infraestrutura do Esquadrão LeoMaestro preparada com sucesso.")
...
def principal():
    ...
    if len(sys.argv) < 2:
        ...
        print(
            "Comandos disponíveis: iniciar, criar_atividade, comunicado_geral, mensagem_direta, listar, atualizar_estado"
        )
        sys.exit(1)
    ...
    if comando == "iniciar":
        ...
        if len(sys.argv) < 4:
            print(
                "Uso: python gerenciador_equipe.py criar_atividade <titulo> <responsavel> [pre_req1,pre_req2,...]"
            )
            sys.exit(1)
        titulo = sys.argv[2]
        ...
        if len(sys.argv) < 4:
            print(
                "Uso: python gerenciador_equipe.py comunicado_geral <remetente> <conteudo>"
            )
            sys.exit(1)
        ...
        if len(sys.argv) < 5:
            print(
                "Uso: python gerenciador_equipe.py mensagem_direta <remetente> <destinatario> <conteudo>"
            )
            sys.exit(1)
        ...
        for ativ in atividades:
            print(
                f"  #{ativ['id']} [{ativ['estado']}] {ativ['titulo']} → {ativ['responsavel']}"
            )

    elif comando == "atualizar_estado":
        if len(sys.argv) < 4:
            print(
                "Uso: python gerenciador_equipe.py atualizar_estado <id> <novo_estado>"
            )
            sys.exit(1)
