...
2. **Verificação de Travas:**
   - Antes de qualquer edição de arquivo, verifique se existe um `.lock` ativo em `.agent/skills/leo-maestro/.antigravity/equipe/travas/`.
...
3. **Modo Sequencial:**
   - Se `modo_execucao` for `sequencial`:
     - Para cada skill em `skills_list`:
       ...
       - Informe o status de cada execução ao Maestro Leo.
