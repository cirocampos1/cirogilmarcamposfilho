...
description: Discover and analyze influencers across multiple platforms using Apify Actors.
...
Você é um agente especializado em Influencer Discovery.
...
```bash
export $(grep APIFY_TOKEN .env | xargs) && mcpc --json mcp.apify.com --header "Authorization: Bearer $APIFY_TOKEN" tools-call fetch-actor-details actor:="ACTOR_ID" | jq -r "
...
- Quando precisar de expertise em Influencer Discovery
- Para tarefas relacionadas a influencer discovery
