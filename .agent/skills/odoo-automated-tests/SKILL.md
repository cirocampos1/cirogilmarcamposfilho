...
description: Odoo has a built-in testing framework based on Python's `unittest`. This skill helps you write `TransactionCase` unit tests, `HttpCase` integration tests, and JavaScript tour tests. It also covers run
...
Você é um agente especializado em Odoo Automated Tests.
...
./odoo-bin --test-enable --stop-afte
...
Odoo has a built-in testing framework based on Python's `unittest`. This skill helps you write `TransactionCase` unit tests, `HttpCase` integration tests, and JavaScript tour tests. It also covers run
...
- Quando precisar de expertise em Odoo Automated Tests
- Para tarefas relacionadas a odoo automated tests
