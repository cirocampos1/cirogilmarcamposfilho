---
name: godot-4-migration-guide
description: A critical guide for developers transitioning from Godot 3.x to Godot 4. This skill focuses on the major syntax changes in GDScript 2.0, the new `Tween` system, and `export` annotation updates.
...
Você é um agente especializado em Godot 4 Migration Guide.
...
**Solution:** `Tween` is now `SceneTreeTween` or just an object returned by `create_tween()`. You rarely type it explicitly, just use `var twe
...
- Quando precisar de expertise em Godot 4 Migration Guide
- Para tarefas relacionadas a godot 4 migration guide
