...
from app.services.statsbomb_parser import get_team_display
...
def _plot_team_pressure_heatmap(pressures, team_name, color, flip=False):
    pitch = get_pitch()
    fig, ax = pitch.draw(figsize=(10, 7))
    fig.set_facecolor('#22312b')
    ax.set_facecolor('#22312b')
    ...
    for pressure in pressures:
        raw_team = pressure.get("possession_team") or pressure.get("team")
        pressure_team = raw_team.get("name") if isinstance(raw_team, dict) else raw_team
        location = pressure.get("location")
        if pressure_team != team_name or not location or len(location) < 2:
            continue
        x, y = location[0], location[1]
        xs.append(120 - x if flip else x)
        ys.append(80 - y if flip else y)

    if xs:
        cmap = LinearSegmentedColormap.from_list(
            f"{team_name}-pressure",
            [(0.0, "#22312b"), (0.35, color), (1.0, "#fff8ea")],
        )
        sns.kdeplot(
            x=xs,
            y=ys,
        ...
        pitch.scatter(xs, ys, s=10, color=color, alpha=0.16, ax=ax, zorder=3)
    ax.set_title(
        f"{get_team_display(team_name)['name']} · {len(xs)} pressões",
        color="white",
        fontsize=14,
    )
    return {
        "team": get_team_display(team_name)["name"],
        "events": len(xs),
        "image": get_base64_image(fig),
    }


def plot_pressure_heatmaps(pressures, home_team, away_team, home_color, away_color):
    return [
        _plot_team_pressure_heatmap(pressures, home_team, home_color, flip=False),
        _plot_team_pressure_heatmap(pressures, away_team, away_color, flip=True),
    ]
