...
def plot_player_heatmap(events, player_id):
    ...
    for e in events:
        ...
        if not p_info:
            continue
        pid = clean_id(p_info.get("id") if isinstance(p_info, dict) else e.get("player_id"))
        if pid == player_id:
            loc = e.get("location")
    ...
    if player_x:
        sns.kdeplot(x=player_x, y=player_y, fill=True, cmap="mako", alpha=0.6, ax=ax, bw_adjust=0.5)
        
    ax.set_title(f"Mapa de Calor: {pname}", color='white', fontsize=14, pad=10)
    return get_base64_image(fig)


def plot_player_passmap(events, player_id):
    player_id = clean_id(player_id)
    player_passes = []
    pname = "Jogador"
    ...
    for p in player_passes:
        ...
        x1, y1 = loc[0], loc[1]
        x2, y2 = end_loc[0], end_loc[1]
        
        outcome = p.get("pass_outcome")
        is_completed = not outcome
        
        is_progressive = is_completed and ((x2 - x1) >= 10)
        
        if is_progressive:
            pitch.arrows(x1, y1, x2, y2, color='#10b981', width=2, headwidth=5, headlength=5, ax=ax, zorder=3, alpha=0.9)
        elif is_completed:
            pitch.arrows(x1, y1, x2, y2, color='#3b82f6', width=1.5, headwidth=4, headlength=4, ax=ax, zorder=2, alpha=0.7)
        else:
            pitch.arrows(x1, y1, x2, y2, color='#ef4444', width=1.2, headwidth=3, headlength=3, ax=ax, zorder=1, alpha=0.4, linestyle='--')
            
    ax.set_title(f"Mapa de Passes: {pname} (Verde: Progressivos, Azul: Completos, Vermelho: Incompletos)", color='white', fontsize=14, pad=10)
    return get_base64_image(fig)
