...
def calculate_detailed_team_stats(events, home_team, away_team):
    stats = {
        "home": {
            "shots": 0, "shots_on_target": 0, "passes": 0, "passes_completed": 0,
            "possession": 0, "corners": 0, "fouls": 0,
            "yellow_cards": 0, "red_cards": 0, "offsides": 0,
            "clearances": 0, "interceptions": 0, "tackles": 0, "blocks": 0
        },
        "away": {
            "shots": 0, "shots_on_target": 0, "passes": 0, "passes_completed": 0,
            "possession": 0, "corners": 0, "fouls": 0,
    ...
    total_possession = {"home": 0, "away": 0}
    ...
    for e in events:
        ...
        # Possession tracking (each event counts as a possession moment)
        if t_key:
            total_possession[t_key] += 1

        # Team-aware stats (use possession_team)
        if t_key:
            etype = e.get("type")
            ...
            if etype == "Shot":
                ...
                if outcome in (None, "Goal", "Saved", "Post", "Blocked", "Wayward"):
                    stats[t_key]["shots_on_target"] += 1

            if etype == "Pass":
                stats[t_key]["passes"] += 1
                outcome = e.get("pass_outcome")
                if not outcome:
                    stats[t_key]["passes_completed"] += 1
                if e.get("pass_corner"):
                    stats[t_key]["corners"] += 1

            if etype == "Foul Committed":
                stats[t_key]["fouls"] += 1

            if etype == "Bad Behaviour":
                card = e.get("bad_behaviour_card")
                if card == "Yellow Card":
                    stats[t_key]["yellow_cards"] += 1
                elif card == "Red Card":
                    stats[t_key]["red_cards"] += 1

            if etype == "Offside":
                stats[t_key]["offsides"] += 1

            if etype == "Clearance":
                stats[t_key]["clearances"] += 1

            if etype == "Interception":
                stats[t_key]["interceptions"] += 1

            if etype == "Duel":
                stats[t_key]["tackles"] += 1

            if etype == "Block":
                stats[t_key]["blocks"] += 1

    # Calculate possession percentages
    total_events = total_possession["home"] + total_possession["away"] or 1
    stats["home"]["possession"] = round(total_possession["home"] / total_events * 100, 1)
    stats["away"]["possession"] = round(total_possession["away"] / total_events * 100, 1)
